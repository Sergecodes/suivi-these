...
const StudentDashboard = (props) => {
  ...
  return (
    <section className="studentDashboard px-2" style={files.clicked===false &&width<1015 ?{display:"none"}:{}}>
       ...
       <ImCross className="dashboardCross" onClick={()=>{dispatch(setClicked())}} style={files.clicked===true&&width<1015?{}:{display:"none"}}/>
        <div className=""  >
            <div className="d-flex justify-content-center">
            <img className="studentPicture" src={EtudiantData[0].urlPhotoProfil} alt=""/> 

            
            </div>
            <div className="studentInfo" style={{lineHeight:"1.4"}} >
                <p className="fs-6" style={{}}>{EtudiantData[0].nom}</p>
                <p className="fs-6" style={{}}>{EtudiantData[0].prenom}</p>
